...
\ezimeti{
\item For any set $A$, the number of elements in it is called the
\uterm{cardinality} of $A$, depicted as $|A|$.
...
\item Given any set $A$, is $A\subseteq A$?

\item Given any set $A$, what can you say about $\emptyset \subseteq A$?
...
\begin{enumerate}
\item[]
\item[a.] Define proper subsethood in terms of '$\subseteq$', you can negate it as '$\not\subseteq$' if you need to. 
\item[b.] Given $A \not\subseteq B$, what can you tell about $A$?
\item[c.] Give a partition of $A = \emptyset$.
\end{enumerate} \end{uexercise} \hrulefill
}
...
\begin{itemize} \item[\ref{ex-part}] Number your trials from 0 to $k$; put $n$
